...
lacks(engine, 'currentPps < CAPTURE_CANCEL_PPS', 'capture recovery is not cancelled at exactly 24 pps')
has(engine, 'currentPps > CAPTURE_CANCEL_PPS', 'capture recovery requires strictly greater than 24 pps')
...
print(f'F31 match automation regression PASS {len(checks)}/{len(checks)}')
for label in checks:
    print('PASS', label)
