...
has(engine, 'AUTO_CAPTURE_DELAY_MS = 5 * 60_000L', 'capture gate is exactly five minutes after T0')
...
has(engine, 'PrimeClient.resumedPackages()', 'stats hunt reads the real resumed package list')
has(engine, 'peerlinkFront', 'opening PeerLink does not abort the score hunt')
...
lacks(overlay, 'button("FT"', 'manual FT button is gone')
...
has(engine, 'attachDetectedStats', 'stats board rows attach after the score is locked')
lacks(engine, 'confirmedCandidate', 'manual FT candidate path is gone')
lacks(engine, 'invalid_manual_ft', 'fake manual FT forfeit is gone')
